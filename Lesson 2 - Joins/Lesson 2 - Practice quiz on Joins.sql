...
FROM region r
JOIN sales_reps s
ON s.region_id = r.id AND s.name LIKE '% K%' AND r.name = 'Midwest'
JOIN accounts a
ON s.id = a.sales_rep_id
ORDER BY a.name;
...
JOIN orders o
ON a.id = o.account_id AND standard_qty > 100;
...
FROM accounts a
JOIN web_events w
ON w.account_id = a.id AND w.account_id ='1001'; /*you can also use a.id ='1001'. it gives the same result*/
